...
Tahoma.prototype.createOrUpdateState = function(key, value, options) {
    ...
    controller.context.getObject(key, function(err, obj) {
		if(err || !obj) {
			...
			controller.context.log.silly('createState ' + key + ' 0=> ' + value);
			// create state
			...
			controller.context.log.silly('setState ' + key + ' 2=> ' + value);	// CJ
			controller.ackStateValues[key] = value;
			controller.context.setState(key, value, true);
		}
...
Tahoma.prototype.log(Tahoma.LogLevel loglevel,  String message) {
	...
	logMsg =  className + "." + methodName + "():" + lineNumber, message;

	switch(loglevel){
		case LogLevel.error:
			controller.context.log.debug(logMsg);
			break;
		case LogLevel.warn:
			controller.context.log.debug(logMsg);
			break;
