...
Tahoma.prototype.sendInternalRequest = function(method, requestPath, payload, callback) {
	...
	// controller.context.log.info(method + " request on " + requestPath + " with payload: " + payload);
	...
	if(requestPath === 'login') {
		controller.context.log.info(method + " request on " + url + " with payload: +++redacted+++");
	} else {
		controller.context.log.info(method + " request on " + url + " with payload:" + JSON.stringify(payload));
	}
    ...
    reqFunc({
		...
		if(!error && response && (response.statusCode === 200 || response.statusCode === 204)) {
			...
			if(controller.loginErrors > 3) {
				...
				controller.context.log.info('loginErrors++');
				controller.loginErrorTimeout = setTimeout(function() {
					controller.loginErrorTimeout = null;
					controller.loginErrors = 0;
					controller.context.log.info('loginErrors = 0');
				}, 60000);
				// perform login and send again
				controller.context.log.info('perform login and send again');
				sendFunc(requestPath, payload, callback);
			}
			...
			if(typeof response !== "undefined") {
				controller.context.log.info("response status: " + response.statusCode + " " + response.statusText);
...
Tahoma.prototype.createOrUpdateState = function(key, value, options) {
	controller.context.log.info('createOrUpdateState: ' + key + ' => ' + value);
    ...
    controller.context.getObject(key, function(err, obj) {
		if(err || !obj) {
			...
			controller.context.log.info('createState ' + key + ' => ' + value);
			// create state
			controller.context.setObject(key, {
				...
				controller.context.log.info('setState ' + key + ' 1=> ' + value); // CJ
				controller.ackStateValues[key] = value;
				controller.context.setState(key, value, true);
			});
		} else {
			controller.context.log.info('setState ' + key + ' 2=> ' + value);	// CJ
			controller.ackStateValues[key] = value;
			controller.context.setState(key, value, true);
		}
...
Tahoma.prototype.updateGateWays = function(nextEntry) {
	...
	controller.context.log.info('Update gateway status for id ' + controller.gatewayList[nextEntry]);
	controller.sendGET('setup/gateways/' + controller.gatewayList[nextEntry], {}, function(err, data) {
		if(!err && data) {
			controller.updateGateWayData([data]);
			controller.context.log.info('Updated gateway status for id ' + controller.gatewayList[nextEntry]);
		}
...
Tahoma.prototype.fetchEvents = function(callback) {
	...
	if(controller.lastRefresh < curTime - (5 * 60 * 1000)) {
		controller.context.log.info('Refreshing device states.');
    ...
    controller.sendPOST("events/" + controller.eventRegisterID + "/fetch", {}, function (err,data) {
        ...
        controller.context.log.info("events/" + controller.eventRegisterID + "/fetch" + "Fetched Data " + JSON.stringify(data));
...
Tahoma.prototype.updateEventState = function(event) {
	controller.context.log.info('Event: ' + JSON.stringify(event));
	...
	controller.sendGET(url, {}, function(err, data) {
		...
		controller.context.log.info(url + " - Fetched Data " + JSON.stringify(data));
...
Tahoma.prototype.updateDeviceActionState = function(event, execId) {
	...
	controller.context.log.info("got action event for device " + devicePath + ': ' + JSON.stringify(event));
	...
	if(undefined !== controller.ackStateValues[key]) {
		...
		controller.context.log.info(key + ' not found in ackStateValues. Values are: ' + JSON.stringify(controller.ackStateValues));
...
Tahoma.prototype.updateDeviceState = function(event) {
	...
	controller.context.log.info("got event for device " + devicePath + '(' + deviceURL + ')');
	if(!devicePath) {
		return;
	}
    ...
    for(var i in states) {
        ...
        controller.context.log.info("found " + devicePath + '.states.' + name + " -> " + value);
		...
		controller.context.log.info('setState ' + key + ' 3=> ' + value);	// CJ
		controller.ackStateValues[key] = value;
        controller.context.setState(key, value, true);
		if('undefined' !== typeof controller.existingSlowStates[key]) {
			controller.context.log.info('setState ' + key + ':slow 4=> ' + value);	// CJ
				...
				controller.context.getState(objid, function(err, state) {
					if(!err && state) {
						...
						if(currentTime - state.ts > 5 * 60 * 1000) {
							controller.context.log.info("found old " + device + " -> " + new Date(state.ts));
		...
		for(let i = 0; i < objKeys.length; i++) {
			...
			for(let d = 0; d < delObjects; d++) {
				if(objid.indexOf(delObjects[d]) === 0) {
					controller.context.log.info("delete state:" + objid, 'debug');
...
Tahoma.prototype.processApplyQueue = function(secondtry) {
	controller.context.log.info('ApplyQueue len: ' + controller.applyQueue.length);
	 if(controller.applyQueue.length < 1) {
		controller.applyProcessing = false;
		controller.context.log.info('ApplyQueue processing completed.');
		return;
	} else if(controller.applyProcessing && !secondtry) {
		controller.context.log.info('Skipping, ApplyQueue already processing.');
		return;
	}
...
Tahoma.prototype.onExecuteDeviceCommand = function(id, slow) {
	...
	controller.context.log.info("button pressed: " + id);
